...
export function EmotionalCheckIn({ open, title, onComplete, onSkip }: Props) {
  ...
  return <AnimatePresence><motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-50 flex items-end justify-center bg-[#08111a]/70 p-4 backdrop-blur-sm sm:items-center" onMouseDown={(event) => { if (event.target === event.currentTarget) onSkip(); }}>
    <motion.div ref={dialogRef} tabIndex={-1} role="dialog" aria-modal="true" aria-labelledby="emotion-title" initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} className="hud-panel w-full max-w-xl p-5 outline-none sm:p-7">
      ...
      <div className="mt-5 grid grid-cols-2 gap-2 sm:grid-cols-4">{OPTIONS.map((option) => <button key={option.id} type="button" onClick={() => { setEmotion(option.id); setBodyExplorationDone(option.id !== "unsure"); }} aria-pressed={emotion === option.id} className={`min-h-16 rounded-2xl border p-2 text-left transition ${emotion === option.id ? "border-[#F0CF8E] bg-white/14" : "border-white/10 bg-white/[.045] hover:bg-white/10"}`}><span aria-hidden className="text-xl" style={{ color: option.color }}>{option.symbol}</span><span className="mt-1 block text-xs text-[#E7EEF7]">{option.label}</span></button>)}</div>
      {emotion === "unsure" && !bodyExplorationDone ? <div className="mt-5 rounded-2xl border border-white/10 bg-white/[.045] p-4"><p className="text-sm text-[#E7EEF7]">O que você percebe no seu corpo?</p><div className="mt-3 flex flex-wrap gap-2">{BODY_SIGNALS.map((signal) => <button key={signal} type="button" onClick={() => setBodySignals((current) => current.includes(signal) ? current.filter((item) => item !== signal) : [...current, signal])} aria-pressed={bodySignals.includes(signal)} className={`min-h-10 rounded-full border px-3 text-xs ${bodySignals.includes(signal) ? "border-[#F0CF8E] bg-white/14 text-white" : "border-white/10 text-[#C7D6E6]"}`}>{signal}</button>)}</div><p className="mt-3 text-xs leading-relaxed text-[#B8CAD8]">Esses sinais podem aparecer junto da preocupação, irritação ou cansaço. Só você pode dizer o que combina mais com este momento.</p><button type="button" onClick={() => setBodyExplorationDone(true)} className="hud-btn-secondary mt-3 px-4 text-xs">Continuar</button></div> : null}
      {emotion && bodyExplorationDone ? <div className="mt-5"><p className="text-sm text-[#E7EEF7]">Quanto isso está grande agora?</p><div className="mt-3 flex items-end justify-between gap-2" role="group" aria-label="Intensidade de um a cinco">{[1,2,3,4,5].map((value) => <button key={value} type="button" onClick={() => setIntensity(value)} aria-pressed={intensity === value} aria-label={`Intensidade ${value} de 5`} className="flex min-h-12 flex-1 items-end justify-center rounded-xl border border-white/10 bg-white/[.045] pb-2"><span className="w-5 rounded-full transition" style={{ height: `${12 + value * 6}px`, background: value <= intensity ? "#E8C77A" : "rgba(255,255,255,.18)" }} /></button>)}</div></div> : null}
      <div className="mt-6 flex items-center justify-between gap-3"><button type="button" onClick={onSkip} className="min-h-11 px-2 text-sm text-[#B8CAD8] underline underline-offset-4">Agora não</button><button type="button" disabled={!emotion || !bodyExplorationDone} onClick={() => emotion && onComplete(emotion, intensity)} className="hud-btn-primary px-5 text-sm font-semibold disabled:cursor-not-allowed disabled:opacity-40">Entrar na floresta</button></div>
    </motion.div>
