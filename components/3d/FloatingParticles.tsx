...
export function FloatingParticles({ count, reduceMotion, emphasis }: FloatingParticlesProps) {
  ...
  const geometry = useMemo(() => {
    ...
    for (let index = 0; index < count; index += 1) {
      ...
      sizes[index] = 5 + random01(baseSeed + 3.23) * 14; // More size variance
  ...
  useFrame(({ clock }) => {
    ...
    for (let index = 0; index < count; index += 1) {
      ...
      if (y > 7.5) {
        y = -0.45 + random01(index * 3.19 + Math.floor(elapsed * 0.7)) * 0.7;
      }
      ...
      <shaderMaterial
        ...
        fragmentShader={`
          ...
          void main() {
            ...
            float softEdge = smoothstep(0.52, 0.02, distanceToCenter);
            float core = smoothstep(0.22, 0.0, distanceToCenter);

            // Warmer sunset dust particles
            vec3 warm = vec3(1.0, 0.85, 0.65);
            vec3 cool = vec3(0.85, 0.88, 0.92);
            vec3 color = mix(cool, warm, vTone * 0.85 + 0.15);
            color *= 0.76 + core * 0.55;
