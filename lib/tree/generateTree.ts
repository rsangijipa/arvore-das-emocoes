...
export function generateFractalTree(options: GenerateTreeOptions): TreeData {
  ...
  function grow(
    ...
    for (let child = 0; child < childCount; child += 1) {
      ...
      const outwardWeight = depth === 0 ? 0.94 : depth === 1 ? 0.78 : 0.54;
      const verticalLift = depth === 0 ? 0.3 : depth === 1 ? 0.36 : 0.42;
      ...
      const taperLevels = [1.0, 0.72, 0.5, 0.32, 0.18, 0.1];
  ...
  const startRadius = options.initialRadius * 0.68;

  for (let fork = 0; fork < primaryForkCount; fork += 1) {
    const forkHeight = 0.33 + fork * 0.11 + random() * 0.09;
    const clampedForkHeight = Math.min(0.74, forkHeight);
